...
def index(request):
    if request.method == "GET":
        ...
        quizzes = Quiz.objects.all().order_by("?").values_list("id", flat=True)[:limit]
...
def detail(request, quiz_id):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return Response(
                {"error": "Invalid answer ID"}, status=status.HTTP_400_BAD_REQUEST
            )

        correct_answer = QuizItem.objects.filter(quiz=quiz, is_correct=True).first()

        return Response(
            {"is_correct": is_correct, "correct_answer": correct_answer.choice_text}
        )
