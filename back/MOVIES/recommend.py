...
def recommend_movies(user_id, title):
    try:
        ...
        tfidf = TfidfVectorizer(
            stop_words=["이", "는", "에서", "또", "더", "이상", "을", "가", "한", "의"]
        )
        ...
        highest_popularity_movies = recommended_movies.values("title").annotate(
        ...
        for movie in recommended_titles:
            ...
            highest_popularity_movie = (
                highest_popularity_movies.filter(title=movie)
                .order_by("-max_popularity")
                .first()
            )
            if highest_popularity_movie:
                movie_instance = recommended_movies.filter(
                    title=movie, popularity=highest_popularity_movie["max_popularity"]
                ).first()
