...
from .tmdb_sync import sync_movies
from .tmdb_discover import discover_recent_movie_ids
...
def movie_genre(request, genre_id):
    ...
    if cached_data:
        ...
        genre_movie = Movie.objects.filter(
            genres=genre_id, release_date__lte=date.today()
...
def movie_sort(request):
    ...
    if cached_data:
        ...
        if key == "popular":
            ...
            sort_movies = Movie.objects.filter(release_date__lte=date.today()).order_by(
            ...
            sort_movies = (
                Movie.objects.filter(release_date__lte=date.today(), vote_count__gte=m)
                .annotate(
            ...
            return Response(
                {"error": "Invalid sort number"}, status=status.HTTP_400_BAD_REQUEST
            )
        # 캐시 저장 (1일 동안 캐시)
